...
def test_vision_receiver(interface_ip="0.0.0.0", timeout=0.3):
    ...
    receiver = VisionReceiver(
        interface_ip=interface_ip, ip="224.5.23.2", portVision=10020
    )
    ...
    (f"Iniciando monitoramento completo com timeout de {timeout:.1f}s por ciclo...\n")
    ...
    try:
        while True:
            ...
            (
                f"=== ESTADO COMPLETO DO CAMPO — câmeras recebidas: {len(received_cameras)} ===\n"
            )
            pprint(
                field.get_state()
            )  # Exibe todos os dados atuais do campo (robôs, bola, etc.)
