...
class Pass(py_trees.behaviour.Behaviour):
...
#                                    POSSE_BOLA                                         #
# ---------------------------------------------------------------------------------------#
class Team_Has_ball_posetion(
    py_trees.behaviour.Behaviour
):  # TODO se o will conseguir fazer uma unica variavel que verifica se o time tem posse, e muda caso um dos robos encontre a bola...
...
class is_in_left_side(py_trees.behaviour.Behaviour):
    ...
    def update(self) -> py_trees.common.Status:
        pos = _bb.get(f"{self.robot.robot_id}{positions_values.quadrant}")
        if (pos is not None) and (pos > 6):
            return py_trees.common.Status.SUCCESS
        return py_trees.common.Status.FAILURE
