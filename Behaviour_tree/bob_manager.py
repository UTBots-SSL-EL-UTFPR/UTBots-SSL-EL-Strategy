...
# Árvores específicas (devem existir em outro branch ou serão adicionadas depois)
from Behaviour_tree.trees.stop.stop_tree import get_stop_tree
from Behaviour_tree.trees.halt.halt_tree import get_halt_tree
from Behaviour_tree.trees.penalty.penalty_tree import get_penalty_tree
from Behaviour_tree.trees.penalty_defense.penalty_defense_tree import get_penalty_defense_tree
...
class BobManager:
    ...
    def __init__(self):
        ...
        # Gestão dinâmica de modos / árvores
        self._active_modes: Dict[TeamID, str] = {}
        self._btrees: Dict[TeamID, pt.trees.BehaviourTree] = {}
    ...
    def update_all(self):
        ...
        for rid, bob in self.bobs.items():
            ...
            mode = self._determine_mode(rid)
            prev = self._active_modes.get(rid)
            if mode != prev:
                root = self._build_tree_for_mode(bob, mode)
                self._btrees[rid] = pt.trees.BehaviourTree(root)
                self._active_modes[rid] = mode
            btree = self._btrees.get(rid)
            if btree:
                try:
                    btree.tick()
                except Exception as e:
                    print(f"[BobManager] Erro ao tickar árvore ({rid}): {e}")

    def tick_all(self):
        # Compatibilidade: tikar manualmente (útil em testes)
        for btree in self._btrees.values():
            try:
                btree.tick()
            except Exception as e:
                print(f"[BobManager] Erro tick_all: {e}")

    def get_bob(self, robot_id):
        return self.bobs.get(robot_id)

    def get_tree(self, robot_id):
        btree = self._btrees.get(robot_id)
        return btree.root if btree else None

    # ---------------------------------------#
    #      Gestão de modos / Behavior Trees  #
    # ---------------------------------------#

    def _determine_mode(self, rid: TeamID) -> str:
        """Mapeia flags existentes no blackboard para um modo de árvore.

        Prioridade:
          1. halt            -> robô está travado (IS_STUCK)
          2. penalty_defense -> adversário tem bola e ela está no campo defensivo (placeholder simplificado)
          3. penalty_attack  -> equipe tem bola e intenção de chute (TEAM_KICK)
          4. stop            -> posse neutra (nenhum dos lados tem)
          5. normal          -> default
        """
        bb = self.blackboard
        # Flags principais
        team_has_ball = bb.get(BlackboardKeys.Flags.BallPossession.TEAM_HAS_BALL)
        foes_have_ball = bb.get(BlackboardKeys.Flags.BallPossession.FOES_HAVE_BALL)
        team_kick = bb.get(BlackboardKeys.Flags.KickActions.TEAM_KICK)
        # Campo defensivo (se não existe flag equivalente aqui ainda, pode ser adicionada futuramente)
        ball_in_def_half = bb.get(getattr(BlackboardKeys.Flags, 'BALL_IN_DEFENSIVE_HALF', 'ball_in_defensive_half'))

        # Travado (duas formas possíveis de chave: nome ou índice)
        stuck = bb.get(f"{rid.name}{BlackboardKeys.Flags.Navigation.IS_STUCK}") or \
                bb.get(f"{rid.value}{BlackboardKeys.Flags.Navigation.IS_STUCK}")

        if stuck:
            return "halt"
        if foes_have_ball and ball_in_def_half:
            return "penalty_defense"
        if team_has_ball and team_kick:
            return "penalty_attack"
        if (not team_has_ball) and (not foes_have_ball):
            return "stop"
        return "normal"

    def _build_tree_for_mode(self, bob: Bob, mode: str) -> pt.behaviour.Behaviour:
        try:
            if mode == "stop":
                return get_stop_tree(bob)
            if mode == "halt":
                return get_halt_tree(bob)
            if mode == "penalty_attack":
                return get_penalty_tree(bob)
            if mode == "penalty_defense":
                return get_penalty_defense_tree(bob)
        except Exception as e:
            print(f"[BobManager] Erro construindo árvore para modo {mode}: {e}")

        class Idle(pt.behaviour.Behaviour):
            def __init__(self):
                super().__init__(name="IdleNormal")
            def update(self):
