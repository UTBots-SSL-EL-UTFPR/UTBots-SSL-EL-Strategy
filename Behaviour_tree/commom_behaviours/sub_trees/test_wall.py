...
from .wall_subtree import get_wall_subtree
from .aux_wall_subtree import get_aux_wall_subtree
...
def create_scenario(all_bobs: list[Bob]):
    ...
    logger.info("Inicio simulacao - Wall Test")
    print("-" * 100)
    return
...
# EXECUTAR: python3 -m Behaviour_tree.commom_behaviours.sub_trees.test_wall
if __name__ == "__main__":
    ...
    # Test main wall
    wall_subtree = get_wall_subtree(kamiji)
    # Test auxiliary wall
    aux_wall_subtree = get_aux_wall_subtree(argenton)
    ...
    while True:
        if time.time() >= print_delay + tPrint:
            # prints_e_logs(kamiji, [argenton, sabkawa])
            tPrint = time.time()
