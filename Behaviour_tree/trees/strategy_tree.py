    ...
    def create_tree(self) -> py_trees.behaviour.Behaviour:
        ...
        with_ball_complex_attack = py_trees.composites.Sequence(
            ...
            children=[
                ...
                s_action_nodes.Set_blackboard_value(
                    "Context: recovery", contexts.is_atack_from_recovery, True
                ),
            ],
        )
        with_ball_slow_attack = py_trees.composites.Sequence(
            name="Ball_possetion: with_ball_slow_attack",
            memory=False,
            children=[
                s_condition_nodes.is_slow_attack(),
                s_action_nodes.Set_blackboard_value(
                    "Context: slow_attack", contexts.is_slow_attack, True
                ),
            ],
        )
        team_has_ball_branch = py_trees.composites.Selector(
            "Team Has Ball",
            True,
            children=[
                with_ball_simple_attack,
                with_ball_complex_attack,
                with_ball_slow_attack,
            ],
        )
        ...
        exemple_of_defense = py_trees.composites.Sequence(
            ...
            children=[
                ...
                s_action_nodes.Set_blackboard_value(
                    "iniciar exemplo defesa", contexts.is_defense_exemple, True
                ),
            ],
        )
        foes_has_ball_branch = py_trees.composites.Selector(
            "foes Have Ball", True, children=[exemple_of_defense]
        )
