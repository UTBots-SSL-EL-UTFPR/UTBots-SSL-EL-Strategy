...
SAFE_BALL_DISTANCE = 500  # 50 cm em milímetros (projeto usa mm)


def get_halt_tree(robot: Bob) -> pt.behaviour.Behaviour:
    """Arvore: GoToDefaultDefensePos (lento e 50cm da bola)."""
    goto_default_defense = GoToDefaultDefensePos(robot)
    root = pt.composites.Sequence(name="HaltTree", memory=False, children=[goto_default_defense])
    return root
...
class GoToDefaultDefensePos(pt.behaviour.Behaviour):
    ...
    def update(self) -> pt.common.Status:
        ...
        if target.distance_to(ball) < SAFE_BALL_DISTANCE:
            ...
            target = Pose2D(ball.x + (dx / norm) * SAFE_BALL_DISTANCE,
                            ball.y + (dy / norm) * SAFE_BALL_DISTANCE)
