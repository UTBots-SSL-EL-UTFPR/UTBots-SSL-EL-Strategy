...
from Behaviour_tree.helpers.field_helper import (
    FIELD_X_MAX,
    FIELD_X_MIN,
    FIELD_Y_MAX,
    FIELD_Y_MIN,
...
def get_expulso_tree(robot: Bob) -> pt.trees.BehaviourTree:
    is_expulsed = IsExpelled(robot)
    go_off_field = GoOffField(robot)
    tree = pt.composites.Sequence(
        name="ExpulsoTree", memory=False, children=[is_expulsed, go_off_field]
    )
...
class GoOffField(pt.behaviour.Behaviour):
    ...
    def update(self) -> pt.common.Status:
        pos = (
            ...
            if self.robot and self.robot.state
            else Pose2D(0, 0)
        )
        x, y = (pos.x, pos.y)  # compatível com tupla ou Pose2D
