...
class DefenderStrategyHelper:
    ...
    def get_base_position_by_id(cls, robot_id: TeamID) -> Pose2D:
        ...
        base_positions = {
            ...
            # Defensor superior
            TeamID.Argenton: Pose2D(
                goal_center.x + sign * (offset_inside_field + 600), 800
            ),
            # Defensor inferior
            TeamID.SabKawa: Pose2D(
                goal_center.x + sign * (offset_inside_field + 600), -800
            ),
        }
    ...
    def get_aggressive_marking_pose(cls) -> Pose2D | None:
        ...
        target_pos = GeometryHelper.calculate_point_on_line(
            origin=ball_pos, target=our_goal, radius=APPROACH_OFFSET
        )

        # 2. Calcula o ângulo real desejado (virado para o gol adversário)
        real_angle = math.atan2(
            opponent_goal.y - ball_pos.y, opponent_goal.x - ball_pos.x
        )
    ...
    @classmethod
    def get_aggressive_marking_pose_avoiding_ball(
        cls, current_robot_pose: Pose2D
    ) -> Pose2D | None:
        """
        ...
        goal_vec = np.array(
            [FieldHelper.get_team_goal_center().x, FieldHelper.get_team_goal_center().y]
        )
        ...
        if cls.bola_no_caminho(robot_vec, goal_vec, ball_vec, margem=APPROACH_OFFSET):
            ...
            safe_point = cls.ponto_de_desvio(
                robot_vec, goal_vec, ball_vec, offset=APPROACH_OFFSET
            )
    ...
    @classmethod
    def get_safe_marking_pose(
        cls, robot_pos: Pose2D, target_pos: Pose2D, ball_pos: Pose2D
    ) -> Pose2D:
        """
        ...
        if 0 < proj < traj_dist:
            ...
            if lateral_dist < (ROBOT_RADIUS + BALL_AVOID_MARGIN):
                ...
                if np.dot(perp, to_ball) < 0:
                    perp = -perp
                safe_point = np.array([ball_pos.x, ball_pos.y]) + perp * (
                    ROBOT_RADIUS + BALL_AVOID_MARGIN
                )
    ...
    @classmethod
    def get_safe_target_pose(
        cls, robot_pose: Pose2D, lateral_offset: float = 300.0
    ) -> Pose2D:
        """
        ...
        ball_pos = cls._ws.get_ball_position()
        if not ball_pos:
            return cls.get_aggressive_marking_pose()  # type: ignore
        ...
        traj_vec = np.array(
            [strategic_target.x - robot_pose.x, strategic_target.y - robot_pose.y]
        )
        ...
        ball_to_target = np.array(
            [strategic_target.x - ball_pos.x, strategic_target.y - ball_pos.y]
        )
    ...
    @classmethod
    def get_safe_target_pose_between_ball_and_our_goal(
        cls, robot_pose: Pose2D, lateral_offset: float = 200.0
    ) -> Pose2D:
        """
        ...
        traj_vec = np.array(
            [strategic_pose.x - robot_pose.x, strategic_pose.y - robot_pose.y]
        )
        traj_norm = traj_vec / (np.linalg.norm(traj_vec) + 1e-6)
        ...
        ball_to_target = np.array(
            [strategic_pose.x - ball_pos.x, strategic_pose.y - ball_pos.y]
        )
        if np.cross(traj_norm, ball_to_target) < 0:
            perp = -perp
