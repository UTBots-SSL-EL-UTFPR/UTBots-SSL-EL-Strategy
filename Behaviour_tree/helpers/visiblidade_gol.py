...
def calc_visible_bobs(
   x0, y0, obstacles_coord, gol_center, theta_max, theta_min
    ...
    for i in range(len(obstacles_coord)):
        ...
        if (xr < x_gol and xr < x0) or (xr > x_gol and xr > x0) or (xr == x0):
           continue
        m = (yr - y0) / (xr - x0)
...
return _type_: float (módulo do ângulo em radianos)
"""
def limits_of_visibility(obstacles: list[Pose2D], p0: Pose2D, gol_center: Pose2D) -> list[float]:
    ...
    if(kick_angle[1] - kick_angle[0]) >= math.radians(VISIBILIDADE_MIN):
        return kick_angle
    else:
        return [0,0]
