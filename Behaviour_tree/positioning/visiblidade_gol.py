...
def haIntersecao(xr, yr, x0, y0, angulo):
    ...
    delta = (
        b**2 - 4 * a * c
    )  # Se o delta for negativo, a reta não cruza a circunferência
    if delta >= 0:
        return True
    return False
...
def calc_visible_bobs(
    x0, y0, obstacles_coord, x_gol, y_golMin, y_golMax, theta_max, theta_min
):
    visible_bobs = []
    for i in range(len(obstacles_coord)):
        ...
        if (intersec_y >= y_golMin and intersec_y <= y_golMax) or (
            ...
            theta_bottom, theta_top = ang_tangent_lines(x0, y0, xr, yr, x_gol)  # type: ignore
...
def is_visible(obstacles, p0, x_gol, y_golMin, y_golMax):
    ...
    visible_bobs = calc_visible_bobs(
        x0, y0, obstacles, x_gol, y_golMin, y_golMax, theta_max, theta_min
    )
    removePontosCegos(visible_bobs)
    visible_bobs.sort(
        key=lambda bob: bob.theta_bottom
    )  # Organiza a lista em ordem crescente do ângulo da reta tangência à parte inferior do bob
