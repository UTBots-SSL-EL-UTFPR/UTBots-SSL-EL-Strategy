...
class RefereeReceiver(Receiver):
    ...
    def __init__(self):
        ...
        super().__init__(
            multicast_ip=config.referee_receiver_ip,
            port=config.referee,
            interface_ip=config.interface_ip_referee,
        )
        ...
        self._thread = threading.Thread(
            target=self.receive_raw, daemon=True
        )  # thread que escuta pacotes em segundo plano
        ...
        while True:
            try:
                ...
                self.latest_parsed = self.parser.parse(
                    data
                )  # usa o parser para decodificar os bytes do pacote em um objeto pyhton com campos acessiveis
            except Exception as e:
                (f"[RefereeReceiver] Erro ao receber pacote: {e}")
