...
class RefereeReceiver(Receiver):
    ...
    def __init__(self):
        if not hasattr(self, "sock"):
            super().__init__(ip="0.0.0.0", port=10003) #TODO, coloquei qualquer valor aqui pro ip e port
    ...
    def receive(self):
        print("Aguardando pacote do Game Controller...")
        data, _ = self.sock.recvfrom(65535)
        print(f"Pacote recebido com {len(data)} bytes.")
        referee = referee_pb.Referee()
        referee.ParseFromString(data)
        print("Mensagem parseada com sucesso.")
        return referee
